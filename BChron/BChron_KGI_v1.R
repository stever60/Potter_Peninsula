...
# SECTION 1: import datasets & initial plots ------------------------------
...
# **Redo this section in Tidyverse and filter to make subset groupings below**

# Read in data in base R 

# Fildes paper - Artigas terrestrial mosses, Artigas-Hall2010, Artigas-marine
x2 <- read.csv("ART_Terr20.csv")
x2.1 <- read.csv("ART_Hall_Terr20.csv")
x4 <- read.csv("ART_Mar20666.csv")
# Potter-Advance, Fildes-Advance, KGI-Advances, KGI-retreat, KGI-Aquatic-moss, SSI-Aquatic-moss
x6 <- read.csv("Advance_Potter.csv")
x7 <- read.csv("Advance_Fildes.csv")
x8 <- read.csv("Advance_KGI.csv") #Advance_KGI is the same dataset as Advance_SSI as all ages are on KGI
...
# Calibrate the age data
# Data classed as normal in calcurves isn't calibrated
...
xAges2_sum <- summary (xAges2, prob = 95.4)
...
# Fildes paper - Artigas terrestrial mosses, Artigas-Hall2010, Artigas-marine
xDens2 <- with(x2, BchronDensity(ages = ages,ageSds = ageSds,calCurves = calCurves, numMix = 10)) #final plot
xDens2.1 <- with(x2.1, BchronDensity(ages = ages,ageSds = ageSds,calCurves = calCurves, numMix = 40)) #final plot
...
#write.csv(xDens2_output,"/Users/Steve/Dropbox/BAS/Data/R/BChron/Output/xDens2_densities.csv", row.names = FALSE)
...
p2 <- plot(xDens2, main="Artigas Terrestrial Moss in Moraines (shcal20) [this study]",
           xlab="Age [a BP]", xlim = c(-100, 4000),
...
p_axis <- c(axis(1, seq(0,4000,500), tck=-0.04), axis(1, seq(-100,4000,100),labels=rep("",42), tck=-0.02),
            axis(2, seq(0,0.01,0.002), labels=rep("",6), tck=-0.04), 
            axis(3, seq(0,4000,500), labels=rep("",9), tck=0.04),axis(3, seq(-100,4000,100),labels=rep("",42), tck=0.02),
            axis(4, seq(0,0.01,0.002), labels=rep("",6), tck=0.04))
par(new = FALSE) #remove hold on the plot frame to add median values

p2.1 <- plot(xDens2.1, main="Artigas-Valle Norte-Valle Klotz Terrestrial Moss in Moraines (shcal20) [this study & Hall, 2007]",
             xlab="Age [a BP]", xlim = c(-100, 4000),
             tck=-0.04, panel.first = grid(nx = NULL, col = "lightgray", lty = "dotted"))
par(new = TRUE) #hold the plot frame to add median values
p_axis <- c(axis(1, seq(0,4000,500), tck=-0.04), axis(1, seq(-100,4000,100),labels=rep("",42), tck=-0.02),
...
p4 <- plot(xDens4, main="Artigas Marine (marine20 DR=666±76)",
...
par(new = TRUE) #hold the plot frame to add median values
p_axis <- c(axis(1, seq(0,12000,1000), tck=-0.04), axis(1, seq(-100,12000,100),labels=rep("",122), tck=-0.02),
            axis(2, seq(0,0.015,0.001), labels=rep("",16), tck=-0.04), 
            axis(3, seq(0,12000,1000), labels=rep("",13), tck=0.04),axis(3, seq(-100,12000,100),labels=rep("",122), tck=0.02),
            axis(4, seq(0,0.015,0.001), labels=rep("",16), tck=0.04))
par(new = FALSE) #remove hold on the plot frame to add median values
p7 <- plot(xDens7, main="Fildes Peninsula: advance",
           xlab="Age [a BP]", xlim = c(-100, 12000),
           tck=-0.04, panel.first = grid(nx = NULL, col = "lightgray", lty = "dotted"))
par(new = TRUE) #hold the plot frame to add median values
p_axis <- c(axis(1, seq(0,12000,1000), tck=-0.04), axis(1, seq(-100,12000,100),labels=rep("",122), tck=-0.02),
...
#Retreat
p9 <- plot(xDens9, main="King George Island: retreat",
           xlab="Age [a BP]", xlim = c(-100, 12000),
           tck=-0.04, panel.first = grid(nx = NULL, col = "lightgray", lty = "dotted"))
par(new = TRUE) #hold the plot frame to add median values
p_axis <- c(axis(1, seq(0,12000,1000), tck=-0.04), axis(1, seq(-100,12000,100),labels=rep("",122), tck=-0.02),
...
p10 <- plot(xDens10, main="SSI: retreat",
            xlab="Age [a BP]", xlim = c(-100, 12000),
            tck=-0.04, panel.first = grid(nx = NULL, col = "lightgray", lty = "dotted"))
par(new = TRUE) #hold the plot frame to add median values
p_axis <- c(axis(1, seq(0,12000,1000), tck=-0.04), axis(1, seq(-100,12000,100),labels=rep("",122), tck=-0.02),
...
#clear plot window - to check plotting OK on screen first
dev.off()
#screen plot layout 
layout(matrix(1:4, ncol=1)) # Set up layout
par(mfrow=c(4,1))  # this sets up the graphics window to expect a 1x3 layout 
par(mar=c(4,4,2,2), oma=c(1,1,2,1),xaxs='i') # Set up internal margins
#xas = "i" tells plot device not to add 4% extra internal margin (which default plot style in R) 
#Fildes advance
p7 <- plot(xDens7, main="Fildes Peninsula: max advance constraints",
           xlab="Age [a BP]", xlim = c(-100, 12000),
           tck=-0.04, panel.first = grid(nx = NULL, col = "lightgray", lty = "dotted"))
par(new = TRUE) #hold the plot frame to add median values
p_axis <- c(axis(1, seq(0,12000,1000), tck=-0.04), axis(1, seq(-100,12000,100),labels=rep("",122), tck=-0.02),
...
#Lakes Fildes
p15 <- plot(xDens15, main="Fildes Lakes basal ages",
            xlab="Age [a BP]", xlim = c(-100, 12000),
            tck=-0.04, panel.first = grid(nx = NULL, col = "lightgray", lty = "dotted"))
par(new = TRUE) #hold the plot frame to add median values
p_axis <- c(axis(1, seq(0,12000,1000), tck=-0.04), axis(1, seq(-100,12000,100),labels=rep("",122), tck=-0.02),
...
#Cosmo fildes
p13 <- plot(xDens13, main="Fildes Cosmo",
            xlab="Age [a BP]", xlim = c(-100, 12000),
            tck=-0.04, panel.first = grid(nx = NULL, col = "lightgray", lty = "dotted"))
par(new = TRUE) #hold the plot frame to add median values
p_axis <- c(axis(1, seq(0,12000,1000), tck=-0.04), axis(1, seq(-100,12000,100),labels=rep("",122), tck=-0.02),
...
##  SCREEN PLOT as 8 cm width to match Sigmaplot output 
# Clear plots
if(!is.null(dev.list())) dev.off()
...
#Aq Moss Layers - KGI (mostly Fildes)
p11 <- plot(xDens11, main="KGI Aquatic Moss",
            xlab="Age [a BP]", xlim = c(-100, 12000),
            tck=-0.04, panel.first = grid(nx = NULL, col = "lightgray", lty = "dotted"))

#Aq Moss Layers - SSI  - not useful - dominated by Byers Peninsula moss layers from Limnopolar lake
p12 <- plot(xDens12, main="SSI Aquatic Moss",
            xlab="Age [a BP]", xlim = c(-100, 12000),
            tck=-0.04, panel.first = grid(nx = NULL, col = "lightgray", lty = "dotted"))

#Cosmo KGI - not very useful as a mixed model due to large errors 
p14 <- plot(xDens14, main="KGI Cosmo",
            xlab="Age [a BP]", xlim = c(-100, 12000),
            tck=-0.04, panel.first = grid(nx = NULL, col = "lightgray", lty = "dotted"))
...
# 1) Artigas Terrestrial Moss in Moraines (shcal20) [this study]
p2 <- plot(xDens2, main="Artigas terrestrial moss in moraines",
           xlab="Age [a BP]", xlim = c(-100, 4000), cex.main = 1,
           tck=-0.04, panel.first = grid(nx = NULL, col = "lightgray", lty = "dotted"))
par(new = TRUE) #hold the plot frame to add median values
p_axis <- c(axis(1, seq(0,4000,500), tck=-0.04), axis(1, seq(-100,4000,100),labels=rep("",42), tck=-0.02),
...
text(100, 0.0055, "A", font = 2, cex = 1.5,  family = 'sans') 
par(new = FALSE) #remove hold on the plot frame to add median values

#2) Artigas-Valle Norte-Valle Klotz Terrestrial Moss in Moraines (shcal20) [this study & Hall, 2007]
...
p_axis <- c(axis(1, seq(0,4000,500), tck=-0.04), axis(1, seq(-100,4000,100),labels=rep("",42), tck=-0.02),
            ...
            axis(3, seq(0,4000,500), labels=rep("",9), tck=0.04),axis(3, seq(-100,4000,100),labels=rep("",42), tck=0.02),
            axis(4, seq(0,0.012,0.002), labels=rep("",7), tck=0.04))
text(100, 0.0045, "B", font = 2, cex = 1.5,  family = 'sans') 
par(new = FALSE) #remove hold on the plot frame to add median values

# 3) Fildes Peninsula: max. age constraints on glacier (re)advance  [this study, Hall, 2007, Watcham et al., 2011]
p7 <- plot(xDens7, main="Fildes Peninsula: constraints on glacier readvance",
           xlab="Age [a BP]", xlim = c(-100, 8000), cex.main = 1,
           tck=-0.04, panel.first = grid(nx = NULL, col = "lightgray", lty = "dotted"))
par(new = TRUE) #hold the plot frame to add median values
...
# 5) KGI: max. age constraint phases for glacier readvance  [this study, HB et al., sub & Hall, 2007]
...
text(400, 0.0105, "D", font = 2, cex = 1.5,  family = 'sans') 
...
#Lakes Fildes - basal ages - deglaciation plot 
...
#Volcanism - check plot only for Figure 5 at  
...
p_axis <- c(axis(1, seq(0,12000,2000), tck=-0.04), axis(1, seq(0,12000,500),labels=rep("",25), tck=-0.02),
            ...
            axis(3, seq(0,12000,2000), labels=rep("",7), tck=0.04), axis(3, seq(0,12000,500),labels=rep("",25), tck=0.02))#,
#axis(4, seq(0,0.008,0.002), labels=rep("",5), tck=0.04))
text(500, 160, "E", font = 2, cex = 1.5,  family = 'sans') 
...
x2 <- read.csv("ART_Terr20.csv")
x2.1 <- read.csv("ART_Hall_Terr20.csv")
x3 <- read.csv("ART_Mar13791.csv")
x4 <- read.csv("ART_Mar20666.csv")
x5 <- read.csv("ART_Mar20.csv")
x6 <- read.csv("Advance_Potter.csv")
x7 <- read.csv("Advance_Fildes.csv")
x8 <- read.csv("Advance_KGI.csv")
x9 <- read.csv("Retreat_KGI.csv")
x10 <- read.csv("Retreat_SSI.csv")
x11 <- read.csv("Aq_Moss_KGI.csv")
x12 <- read.csv("Aq_Moss_SSI.csv")
